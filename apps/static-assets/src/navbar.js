...
// 서브사이트 목록 (name은 i18n 키 — 렌더 시 t()로 치환)
const SUBSITES = [
  { key: 'service.community', url: 'https://community.mannamplace.com', icon: '💬' },
  { key: 'service.news', url: 'https://tnfocnus.mannamplace.com', icon: '📰' },
  { key: 'service.cistan', url: 'https://cistan.mannamplace.com', icon: '🌟' },
  { key: 'service.market', url: 'https://market.mannamplace.com', icon: '🛒' },
  { key: 'service.video', url: 'https://video.mannamplace.com', icon: '🎬' },
  { key: 'service.sound', url: 'https://sound.mannamplace.com', icon: '🎵' },
];
...
export async function initNavbar(options = {}) {
  const {
    ...
    searchPlaceholder = null,  // 검색창 placeholder (미지정 시 t('nav.search'))
    onSearch = null,            // 검색 콜백 (keyword) => {}
    onLogin = null,             // 로그인 성공 콜백 (user, profile) => {}
    onLogout = null,            // 로그아웃 콜백
    showSearchTabs = false,     // 포털용 검색 탭 표시
    searchTabs = [],            // 검색 탭 목록
    i18nExtra = null,           // 앱별 추가 번역 { ko:{}, en:{} }
  } = options;

  // i18n 먼저 초기화 — 이후 렌더링에서 t()를 바로 쓸 수 있도록
  await initI18n({ extra: i18nExtra });
  const placeholder = searchPlaceholder || t('nav.search');
  ...
  navbar.innerHTML = `
    ...
    <div class="mp-navbar__search${variant === 'portal' ? ' mp-navbar__search--portal' : ''}">
      ...
      ${variant === 'portal' ? `<button class="mp-navbar__search-btn" onclick="window._mpNavSearch && window._mpNavSearch(document.getElementById('mp-search-inp').value)">${t('nav.search')}</button>` : ''}
    </div>

    <!-- 비로그인 -->
    <div class="mp-navbar__right" id="mp-auth-area">
      <button class="mp-navbar__btn" onclick="window._mpOpenAuthModal && window._mpOpenAuthModal('login')">${t('nav.login')}</button>
      <button class="mp-navbar__btn mp-navbar__btn--primary" onclick="window._mpOpenAuthModal && window._mpOpenAuthModal('join')">${t('nav.join')}</button>
    </div>

    <!-- 로그인 후 -->
    <div class="mp-navbar__right" id="mp-user-area" style="display:none">
      <div class="mp-navbar__notif" id="mp-notif-btn" onclick="window._mpToggleNotif && window._mpToggleNotif()" title="${t('nav.notifications')}">
      ...
      <div class="mp-navbar__dropdown-wrap">
        ...
        <div class="mp-navbar__dropdown" id="mp-dropdown">
          <div class="mp-navbar__dropdown-head">
            ...
            <div class="mp-navbar__dropdown-badge" id="mp-dd-badge">${t('nav.rank.normal')}</div>
          </div>
          <div class="mp-navbar__dropdown-menu">
            <div class="mp-navbar__dropdown-divider"></div>
            <div style="font-size:9px;color:var(--mp-text3);padding:4px 14px;font-family:var(--mp-font-mono);letter-spacing:1px">${t('nav.serviceSwitch')}</div>
            ${SUBSITES.map(s => `<a class="mp-navbar__dropdown-item" href="${s.url}">${s.icon} ${t(s.key)}</a>`).join('')}
            <div class="mp-navbar__dropdown-divider"></div>
            <a class="mp-navbar__dropdown-item" href="https://account.mannamplace.com">⚙️ ${t('nav.accountSettings')}</a>
            <a class="mp-navbar__dropdown-item mp-navbar__dropdown-item--danger" href="#" id="mp-logout-btn">${t('nav.logout')}</a>
          </div>
        </div>
      </div>
    </div>`;
  ...
  onAuthStateChanged(auth, async (fbUser) => {
    if (fbUser) {
      ...
      const rank = profile?.profiles?.news?.rank || t('nav.rank.normal');
